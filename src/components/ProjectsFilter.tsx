...
export function ProjectsFilter({ projects }: { projects: Project[] }) {
  ...
  const allCategoryLabel = t("filter_all");

  const categories = [allCategoryLabel, ...Array.from(new Set(projects.map((p) => (language === "en" && p.categoryEn ? p.categoryEn : p.category))))];
  const [active, setActive] = useState(allCategoryLabel);

  const list = active === allCategoryLabel
    ? projects
    : projects.filter((p) => {
      const cat = language === "en" && p.categoryEn ? p.categoryEn : p.category;
      return cat === active;
    });
    ...
    <>
      <div className="mt-12 flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          {categories.map((c) => (
            <button
              ...
              onClick={() => setActive(c)}
              className={`inline-flex min-h-11 items-center border px-5 eyebrow transition-colors ${active === c
        ...
        <span aria-live="polite" className="shrink-0 eyebrow text-muted-foreground">
          {String(list.length).padStart(2, "0")} {language === "en" ? "Projects" : "Proje"}
        </span>
      </div>
          ...
          return (
            ...
            >
              <div className="photo-frame bg-muted">
                <Image
                  src={p.cover}
                  alt={`${projectTitle} — ${projectCategory}, ${p.location}`}
                  width={1400}
                  height={1000}
      ...
      {list.length === 0 && (
        <p className="mt-16 text-sm text-muted-foreground">
          {language === "en" ? "No projects published in this category yet." : "Bu kategoride henüz yayınlanmış bir proje yok."}
        </p>
      )}
    </>
  );
